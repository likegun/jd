...
class SearchSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for sel in response.css('li.gl-item'):
            ...
            pics = []
            for pic in sel.css('li.ps-item a'):
                pics.append({
                    'title': ''.join(pic.css('::attr(title)').extract()).strip(),
                    'src': ''.join(pic.css('img::attr(data-lazy-img)').extract()).strip()
                })
            item['pics'] = pics
            item['price'] = ''.join(sel.css('div.p-price strong i::text').extract()).strip()
            item['name'] = ''.join(sel.css('div.p-name em').extract()).strip()
            item['commentCount'] = ''.join(sel.css('div.p-commit a').extract()).strip()
            item['shopUrl'] = ''.join(sel.css('div.p-shop a::attr(href)').extract()).strip()
            item['shopName'] = ''.join(sel.css('div.p-shop a::text').extract()).strip()
            yield item
    ...
    def getPageCount(self, response):
        ...
        self.pageCount = int(
            re.search(r'page_count:\"(\d+)\"', response.body).group(1))
